if __name__ == "__main__":
	for reportId in range(129, 256):
		...
		fd.write(
"""
# I<sup>2</sup>C - Half-Duplex Transfer; {minPayloadSize}-{maxPayloadSize} Bytes Out, 0+ Bytes In
...
|-----------|-------------------------------------------|------------|------------|------------------|----------|
| 1{reportId7bits:07b}  | `rrr`&nbsp;0&nbsp;`R`&nbsp;`c`&nbsp;`ff` | `cccccccc` | `cccccccc` | 0&nbsp;`aaaaaaa` | {maxPayloadSize} bytes  |
...
## Payload
A block of {maxPayloadSize} bytes that will be shifted onto the bus for an *OUT* Report, or the Slave's response for an *IN* Report.  Only the first {minPayloadSize} + *`rrr`* bytes are meaningful, the rest are ignored or undefined.
