...
# I<sup>2</sup>C - Half-Duplex Transfer; 16-23 Bytes Out, 0+ Bytes In
...
|-----------|-------------------------------------------|------------|------------|------------------|----------|
| 10000010  | `rrr`&nbsp;0&nbsp;`R`&nbsp;`c`&nbsp;`ff` | `cccccccc` | `cccccccc` | 0&nbsp;`aaaaaaa` | 23 bytes  |
...
## Payload
A block of 23 bytes that will be shifted onto the bus for an *OUT* Report, or the Slave's response for an *IN* Report.  Only the first 16 + *`rrr`* bytes are meaningful, the rest are ignored or undefined.
